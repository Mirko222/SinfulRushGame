 ...
 * {@link com.badlogic.gdx.scenes.scene2d.ui.SelectBox.SelectBoxStyle#background}.
 * @author mzechner
 * @author Nathan Sweet */
public class VisualSelectBox<T> extends Widget implements Disableable {
    ...
    public VisualSelectBox (Skin skin) {
        this(skin.get(com.badlogic.gdx.scenes.scene2d.ui.SelectBox.SelectBoxStyle.class));
    }

    public VisualSelectBox (Skin skin, String styleName) {
        this(skin.get(styleName, com.badlogic.gdx.scenes.scene2d.ui.SelectBox.SelectBoxStyle.class));
    }

    public VisualSelectBox (com.badlogic.gdx.scenes.scene2d.ui.SelectBox.SelectBoxStyle style) {
        setStyle(style);
    ...
    public void addImage(T item, TextureRegion img, float w, float h) {
        ...
        images.put(item, tmp);
        selectBoxList.list.addImage(item, img, w, h);
    }
    ...
    protected GlyphLayout drawItem (Batch batch, BitmapFont font, T item, float x, float y, float width) {
        ...
        if(images.containsKey(item)) {
            ...
            sprite.draw(batch);
            offx = sprite.getWidth() * sprite.getScaleX() + 10;
        }

        String string = toString(item);
        return font.draw(batch, string, x + offx, y, 0, string.length(), width, alignment, false, "...");
    }
    ...
    static class SelectBoxList<T> extends ScrollPane {
        private final VisualSelectBox<T> selectBox;
        int maxListCount;
        private final Vector2 screenPosition = new Vector2();
        final VisualList<T> list;
        private InputListener hideListener;
        private Actor previousScrollFocus;

        public SelectBoxList (final VisualSelectBox<T> selectBox) {
            ...
            list = new VisualList<T>(selectBox.style.listStyle) {
                @Override
